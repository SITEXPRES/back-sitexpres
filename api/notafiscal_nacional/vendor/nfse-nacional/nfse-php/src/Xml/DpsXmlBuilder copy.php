...
class DpsXmlBuilder
{
    ...
    private function buildValores(DOMElement $parent, ValoresData $data): void
    {
        ...
        if ($data->tributacao) {
            ...
            $this->appendElement($tribMun, 'tribISSQN', $data->tributacao->tributacaoIssqn); // 1 = tributável
            $this->appendElement($tribMun, 'tpRetISSQN', $data->tributacao->tipoRetencaoIssqn); // 1 = não retido

            // Imunidade (somente se existir)
            if ($data->tributacao->tipoImunidade !== null) {
                $this->appendElement($tribMun, 'tpImunidade', $data->tributacao->tipoImunidade);
            }

            // Suspensão (raríssimo no Simples, mas mantido)
            ...
            // =========================
            // <totTrib> (OBRIGATÓRIO PARA SIMPLES)
            // =========================
            $totTrib = $this->dom->createElement('totTrib');

            // Para ME/EPP → SOMENTE pTotTribSN
            $this->appendElement(
                $totTrib,
                'pTotTribSN',
                number_format(
                    $data->tributacao->percentualTotalTributosSN ?? 0,
                    2,
                    '.',
                    ''
                )
